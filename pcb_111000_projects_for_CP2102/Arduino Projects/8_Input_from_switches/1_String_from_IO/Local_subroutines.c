...
p = 0;
while(1){
display_string[0] = display_buffer[num_length - p%(num_length+1)]; 
p += 1;
Num_string_to_display(display_string,0);
SW_Delay_ms(100);

for(int m = 0; m < 7; m++){
display_string[7-m] = display_string[7-m-1];}

while(switch_2_down_local)wdr();
