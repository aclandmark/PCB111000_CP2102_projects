...
while(!(Tx_complete))wdr();}								//Wait here until transmission is complete 		wdr() ADDED
...
PCMSK0 |= 1 << PCINT4;		       					//set up IPC to detect start pulse						wdr() ADDED
while (!(Rx_complete))wdr();}
...
if(PINC5_down){
_delay_ms(12); wdr(); _delay_ms(12); wdr();
if(PINC5_up)return;
for(int m = 0; m < 22; m++){_delay_ms(10); wdr();}			//Switch bounce delay
if(PINC5_up)
{set_Run_BL_bit; }
else{
LED_1_on; LED_2_on;
for(int m = 0; m < 22; m++){_delay_ms(10); wdr();}			//_delay_ms(250);
